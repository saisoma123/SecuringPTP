...
static void timeguard_mru_passive_step(struct clock *c)
{
    ...
    int64_t err_ns = err_out.base_diff_ns;

    if (err_ns > 777101 || err_ns < -777101) {
        err_ns -= 777101;
...
static void timeguard_random_passive_step(struct clock *c)
{
    ...
    if (err_ns > 814548 || err_ns < -814548) {
        err_ns -= 814548;
...
int main(int argc, char *argv[])
{
        ...
        err = 0;
        while (is_running()) {
                // We call either the MRU or random policy every 5 seconds in the daemon loop
                if (clock_poll(clock))
                        break;
